...
-- It can actually be @any@ value :shrug:, we don't have any constraint!
...
unpackCanFold :: (forall f. Foldable f => f a -> c) -> CanFold a -> c
...
instance Foldable CanFold where
  foldr fab b (CanFold a) = foldr fab b a
...
unpackEqPair :: (forall a. Eq a => a -> a -> p) -> EqPair -> p
...
filterPair :: (forall a. Eq a => a -> a -> Bool) -> [EqPair] -> [EqPair]
filterPair f = filter $ unpackEqPair f
